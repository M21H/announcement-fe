...
import { getPosts, postsAction } from '../redux/posts/posts.actions'
...
const Pagination = ({ portionSize = 3 }) => {
	...
	const { pageSize, totalPagesCount, currentPage } = useAppSelector(({ posts }) => posts)
	...
	const handlePageChanged = (page: number) => {
		if (currentPage !== page) {
			...
			dispatch(postsAction.setCurrentPage(page))
			//@ts-ignore
		...
		<div className={classes.container}>
			<ul className={classes.list}>
				...
				{pages
					...
					.map((page, index) => (
						<li
							className={cn({ [classes.active]: currentPage === page }, classes.pageNum)}
							key={index}
